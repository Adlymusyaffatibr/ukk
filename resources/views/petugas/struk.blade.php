...
<div class="bg-white p-6 rounded-xl shadow max-w-xl mx-auto print-area">

    <h2 class="text-xl font-bold mb-4 text-center">KasirApp</h2>

    <img src="{{ asset('warung.png') }}" 
         class="mx-auto mb-4" 
         style="width: 80px; height: auto;">

    <h4 class="font-bold mb-4 text-center">Warung</h4>

    <p><b>Nama :</b> {{ $order->customer_name }}</p>
    <p><b>Tanggal :</b> {{ $order->created_at->format('d-m-Y H:i') }}</p>
    <p><b>Alamat :</b> {{ config('app.store_address') }}</p>
    <p><b>Nama Petugas :</b> {{ $order->user->name }}</p>
    ...
    <p class="flex justify-between">
        <span>Total</span>
        <span>
            Rp {{ number_format($order->total_price + $order->points_used, 0, ',', '.') }}
        </span>
    </p>
    ...
    @if ($order->points_used > 0)
        <p class="flex justify-between text-green-600">
            ...
            <span>
                - Rp {{ number_format($order->points_used, 0, ',', '.') }}
            </span>
        </p>
    @endif
